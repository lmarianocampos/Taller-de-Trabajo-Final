...
%Actualmente son los operarios quienes recorren desde pocos hasta varios kilómetros describiendo la trayectoria que forman las redes de canales y cada una de las compuertas realizan manualmente las mediciones de los valores de parámetros necesarios con los que luego proceden a realizar el calculo correspondiente y establecer dicha compuerta en una determinada posición y de esta manera fijar el caudal de agua deseado. 	 
...
El esquema que se ilustra en la "Figura \ref{fig:canal}", podemos notar que esta compuesto  principalmente por la automatización de cada una de las compuertas que conforman la red de canal, un centro de monitoreo donde reside el sistema de control central para gestionar la entrega de agua de forma remota.
...
Se denomino Celda Primaria a una fracción pequeña del sistema global que en general es el más repetido con frecuencia, y que se encuentra dispuesto en cada una de las compuertas que constituye una red de canal de riego a cielo abierto, como podemos percibir en la "Figura \ref{fig:canal}".
...
\end{figure}
%como cuna segunda instancia un arreglo de dichas actividades y aspectos, y se
%incorporan componentes no considerados tradicionalmente como parte de la operación: la medición y la infraestructura.
